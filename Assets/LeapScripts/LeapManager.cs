...
//	}
	...
	void Awake()
	{
		...
		if(LeapSensorPrefab)
		{
			...
			LeapSensorPrefab.tag = "leapmanager";
			Debug.Log(LeapSensorPrefab);
		}
...
//				throw new Exception("Please connect the LeapMotion sensor!");
			...
			//이거하면 계속 LeapManager가 생겨서 느려지게된다. 
			...
			Finger finger = frame.Finger(fingersCountHandID);
...
//					fScreenTapProgress = 0f;
				Hand leftmostHand = leapFrame.Hands.Leftmost;
				...
				if(leapPointable == null || !leapPointable.IsValid)
				{
					...
					fingersCountFrame = leapPointable.Hand.Fingers.Count;
				}
...
//				}
	...
	public int isHandCount(Hand hand)
	{
